...
课程大纲详情页不会把 EMAP 内部字段名直接作为主界面标签。基本信息按“课程概况”“课程属性”“学分与学时”三个紧凑分组展示：`KCM`、`KCH`、`XF`、`XS`、`SYXS`、`KTJSXS`、`JYKKXQ` 等常见字段会分别显示为“课程名称”“课程号”“学分”“总学时”“实验学时”“课堂教学学时”“建议开课学期”；同一字段的 `_DISPLAY`/名称别名优先合并为可读文字，代码值保留在“系统信息”中，避免把“开课单位（代码）/开课单位”“考核方式（代码）/考核方式”等重复铺开。记录标识、大纲版本等技术字段自动归入“系统信息”。“教材参考 / 先修”章节会将 `XXKC` 识别为先修课程，将 `CKSJJXZY` 识别为课外参考书及资料；不同部署返回的未命名教材字段会按其书目信息（如 `||` 分隔的书名、ISBN、出版社和作者）识别为课程使用的教材，不再笼统显示为“教材参考信息 1/2”。适用专业、先修课程、课程使用的教材和其他说明直接显示；“课程成绩评定方法”中的业务信息也直接展示。未知字段和空字段也不会丢失，而是按需收起；完整响应仍可在“原始数据”、复制和下载 JSON 中核对。
