...
课程大纲详情页不会把 EMAP 内部字段名直接作为主界面标签。`KCM`、`KCH`、`XF`、`XS`、`SYXS`、`KTJSXS`、`JYKKXQ` 等常见字段会分别显示为“课程名称”“课程号”“学分”“总学时”“实验学时”“课堂教学学时”“建议开课学期”；带有 `_DISPLAY`/名称别名时优先显示可读文字，记录标识、大纲版本等技术字段自动归入“系统信息”。“教材参考 / 先修”固定紧跟基本信息展示，适用专业、先修课程、课程使用的教材和其他说明直接显示；“课程成绩评定方法”中的业务信息也直接展示。未知字段和空字段也不会丢失，而是按需收起；完整响应仍可在“原始数据”、复制和下载 JSON 中核对。
