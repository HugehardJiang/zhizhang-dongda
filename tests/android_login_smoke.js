...
assert.match(source, /roundBackground\(getColor\(R\.color\.native_surface\), 0, 0, 12, 12\)/);
...
assert.match(source, /openPortalForReauthentication\(\)[\s\S]*loadUrl\(academicViewerEntryUrl\(\)\)/);
assert.match(source, /isCampusAccess\(\) \? academicCasCallbackUrl\(\) : academicHomeUrl\(\)/);
assert.match(source, /recoverCampusAcademicNotFound/);
assert.match(source, /academic-404-fallback/);
assert.match(source, /\/jwapp\/sys\/homeapp\/\*default\/index\.do/);
assert.match(source, /isAcademicMissingHomeAppPath/);
assert.match(source, /backgroundLoginAttemptedForCurrentFailure && "failed"\.equals\(loginDiagnosticStatus\)/);
assert.match(source, /isAcademicCasTicketUrl\(url\)/);
